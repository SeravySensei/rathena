...
void initChangeTables(void)
{
	...
	add_sc( SO_DIAMONDDUST, SC_FREEZE);
...
//	set_sc( RA_UNLIMIT		, SC_UNLIMIT		, EFST_UNLIMIT		, SCB_DEF|SCB_DEF2|SCB_MDEF|SCB_MDEF2 );
	...
	set_sc( LG_PRESTIGE		, SC_PRESTIGE		, EFST_PRESTIGE		, SCB_DEF|SCB_DEF2);
	...
	set_sc( SC_ENERVATION		, SC__ENERVATION	, EFST_ENERVATION		, SCB_DEX|SCB_AGI );
	set_sc( SC_GROOMY		, SC__GROOMY		, EFST_GROOMY		, SCB_INT|SCB_VIT );
	set_sc( SC_IGNORANCE		, SC__IGNORANCE		, EFST_IGNORANCE		, SCB_STR|SCB_INT );
	set_sc( SC_LAZINESS		, SC__LAZINESS		, EFST_LAZINESS		, SCB_INT|SCB_DEX );
	set_sc( SC_UNLUCKY		, SC__UNLUCKY		, EFST_UNLUCKY		, SCB_AGI|SCB_LUK );
	set_sc( SC_WEAKNESS		, SC__WEAKNESS		, EFST_WEAKNESS		, SCB_AGI|SCB_STR );
	...
	set_sc( SR_GENTLETOUCH_CHANGE		, SC_GT_CHANGE		, EFST_GENTLETOUCH_CHANGE		, SCB_WATK|SCB_ASPD|SCB_DEF2|SCB_MDEF2 );
	set_sc( SR_GENTLETOUCH_REVITALIZE	, SC_GT_REVITALIZE	, EFST_GENTLETOUCH_REVITALIZE	, SCB_MAXHP|SCB_REGEN|SCB_DEF2 );
...
static unsigned short status_calc_agi(struct block_list *bl, struct status_change *sc, int agi)
{
	...
	if (sc->data[SC__ENERVATION])
		agi -= sc->data[SC__ENERVATION]->val3;
	if (agi < 0) agi = 0;
	if (sc->data[SC__UNLUCKY])
		agi += sc->data[SC__UNLUCKY]->val2;
	if (sc->data[SC__WEAKNESS])
...
static unsigned short status_calc_int(struct block_list *bl, struct status_change *sc, int int_)
{
	...
	if (sc->data[SC__IGNORANCE])
		int_ -= sc->data[SC__IGNORANCE]->val3;
	if (int_ < 0) int_ = 0;
	if (sc->data[SC_FRIGG_SONG])
		int_ += sc->data[SC_FRIGG_SONG]->val3;
	if (sc->data[SC__LAZINESS])
		int_ += sc->data[SC__LAZINESS]->val2;
	if (sc->data[SC__GROOMY])
...
/*	if(sc->data[SC_ETERNALCHAOS])
	...
	if (sc->data[SC_GT_CHANGE])
		def2 -= sc->data[SC_GT_CHANGE]->val4;
	if (sc->data[SC_GT_REVITALIZE])
...
static signed short status_calc_mdef2(struct block_list *bl, struct status_change *sc, int mdef2)
{
	...
	if(sc->data[SC_BURNING])
		mdef2 -= mdef2 * 25 / 100;
	if (sc->data[SC_GT_CHANGE])
		mdef2 -= sc->data[SC_GT_CHANGE]->val4;
...
//		if (status_get_race2(bl) == RC2_GVG && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_WEAPONBLOCKING:
			val2 = 2 * val1 + status->luk /7 + status->agi / 10; // Chance
			if (val2 > 50) val2 = 50;
			val4 = tick / 5000;
		...
		case SC_BLOODSUCKER:
			...
			break;
		case SC_SWINGDANCE:
			val3 = 5 * val1; // Walk speed and aspd reduction.
			break;
		case SC_SYMPHONYOFLOVER:
			val3 = 10 * val1; // MDEF Increase in %
			break;
		...
		case SC_PRESTIGE:
			...
			val3 = ((val1 * 15) + (5 * (sd?pc_checkskill(sd,CR_DEFENDER):skill_get_max(CR_DEFENDER)))) * status_get_lv(bl) / 100; // Defence added
			break;
		case SC_BANDING:
		...
		case SC_GT_REVITALIZE:
			...
			val4 = val1 * 15; // STAT DEF increase
			break;
