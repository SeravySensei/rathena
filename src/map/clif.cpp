...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if (pc_checkskill(sd, SG_DEVIL) && !pc_checkskill(sd, SJ_PURIFY) && ((sd->class_&MAPID_THIRDMASK) == MAPID_STAR_EMPEROR || pc_is_maxjoblv(sd)))
...
void clif_parse_ProduceMix(int fd,struct map_session_data *sd){
	...
	if (pc_checkskill(sd, AM_TWILIGHT1)>0) {
		while (produce_idx = skill_can_produce_mix(sd, p->itemId, sd->menuskill_val, 1)) {
			skill_produce_mix(sd, 0, p->itemId, p->material[0], p->material[1], p->material[2], 1, produce_idx - 1);
		};
	}
	else if (produce_idx = skill_can_produce_mix(sd, p->itemId, sd->menuskill_val, 1)) {
		skill_produce_mix(sd, 0, p->itemId, p->material[0], p->material[1], p->material[2], 1, produce_idx - 1);
	};
